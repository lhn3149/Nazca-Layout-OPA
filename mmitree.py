...
def mmi(offset,length, width):
    with nd.Cell(name='myMMI') as mymmi:
        ...
        elm2 = ic.sbend(offset=offset, width=width).put(elm1.pin['b0'])
        elm3 = ic.sbend(offset=-offset,width=width).put(elm1.pin['b0'])
...
offset = 40     # offset between split
...
###### Parameters (Cont) ############
offset_curve = 10       # horizontal offset between 2 adjacent wg
...
x_avg = split.pin['b1'].x + (N+1)/2 * offset_curve      # x location of the horizontal mid-point in the array
length=20     # length of the straight portion before the trombones
distribution = [-73.0125, -72.1125,	-69.4125, -63.1125,	-49.6125, -44.2125,	-22.6125, -11.8125,	-4.6125, 30.4875, 32.2875, 47.5875,	61.9875, 73.6875, 78.1875, 86.2875]
...
for i in range(N):
    ...
    input_pin = "a" + str(i)
    output_pin = "b" + str(i) 
    OPA_x = x_avg + distribution[i] # x-location of OPA array given a distribution 
    i = i + 1
    print("Position of waveguide", i)
    curve = curve_bend(width=width, length1=i*offset_curve, length2=length+i*offset_split).put(split.pin[output_pin])

    # use between N to 2N trombones to compensate for trace difference
    for j in range(2*N):
        path_diff = (N-i)*offset_curve + (N-i)*offset_split
        tromb = trombone(length=path_diff/(2*2*N), radius=5, width=0.3).put()
    curve_x = tromb.pin['b0'].x 
    curve_y = tromb.pin['b0'].y

    strt2 = nd.strt(length = length_OPA, width =width).put(OPA_x, curve_y-50, -90)

    sbend1  = ic.bend_strt_bend_p2p(pin1=tromb.pin['b0'] , pin2=strt2.pin['a0'], radius=10).put()
...
print("trace length of waveguides: ", np.absolute(trace_array-trace_array[0]))
print("Offset split: ", offset_split)
print("Offset cross split", offset_cross_split)
nd.export_gds(filename="nazca_design.gds")